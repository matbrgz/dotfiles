#!/bin/bash
{
	...
	lt " [ DONE ] Instaling Major Requirements ... %s seconds\n" "$((endtime - starttime))"
 ...
 Option: "
	...
	if [ -z "${firstrun}" ] || [ "${firstrun}" == Y ] || [ "${firstrun}" == y ]; then
		...
		lt " [ DONE ] Instaling Common Requirements ... %s seconds\n" "$((endtime - starttime))"
		...
		lt " [ DONE ] Configurating Command Alias ... %s seconds\n" "$((endtime - starttime))"
		...
		lt " [ DONE ] Fix Possible Erros ... %s seconds\n" "$((endtime - starttime))"
	...
	if [ "${instalationtype}" == 1 ]; then
		...
		lt " [ DONE ] Software Instalation List ... %s seconds\n" "$((endtime - starttime))"
